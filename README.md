...
For my first results (after developing the algorithm), recall is a lot higher than precision, which means that when a real POI shows up in the test set, the algorithm is able to identify him/her. But sometimes, it will get some false positives where non-POIs get flagged. In my tester.py results, recall is a little lower than precision, which means that whenever a POI gets flagged in the test set, it's highly likely that it is a real POI and not a false alarm. But sometimes, the algorithm can miss real POIs.
